...
export async function GET(req: NextRequest) {
  ...
  const typesParam = "ORIGINAL"; // e.g. "ORIGINAL" or "DIGITAL,PRINT" or "ALL"
  const types = typesParam.split(",").map(s => s.trim().toUpperCase()) ;

  const payload = await listFavoritesForUser({ userId , types}); // <-- FIXED: provide `types` key
