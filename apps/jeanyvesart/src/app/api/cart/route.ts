...
// apps/*/src/app/api/cart/route.ts
import { NextRequest, NextResponse } from "next/server";
import { getCustomerIdFromRequest } from "@acme/core/utils/guest";
import { addOriginalToCart, removeOriginalFromCart, listCartItemsForCustomer} from "@acme/server/services/cart";
...
// GET /api/cart?types=ORIGINAL|DIGITAL|PRINT|DIGITAL,PRINT|ALL&liveDesignPreview=1
export async function GET(req: NextRequest) {
  const { userId, guestId } = await getCustomerIdFromRequest(req);
  const sp = req.nextUrl.searchParams;
  const typesParam = "ORIGINAL"; // sp.get("types"); // e.g. "ORIGINAL", "DIGITAL,PRINT", "ALL", null
  const types = !typesParam || typesParam === "ALL" ? undefined : typesParam;
  const liveDesignPreview = sp.get("liveDesignPreview") === "1";

  const items = await listCartItemsForCustomer({
...
export async function POST(req: NextRequest) {
  const { userId, guestId } = await getCustomerIdFromRequest(req);
  const { productId, originalVariantId } = await req.json();
  if (!productId) return NextResponse.json({ error: "Missing productId" }, { status: 400 });

  // For ORIGINAL we ignore client-sent variant id and look it up (idempotent add)
  const result = await addOriginalToCart({ userId: userId ?? undefined, guestId: guestId ?? undefined }, String(productId));
...
export async function DELETE(req: NextRequest) {
  ...
  await removeOriginalFromCart({ userId: userId ?? undefined, guestId: guestId ?? undefined }, String(productId));
