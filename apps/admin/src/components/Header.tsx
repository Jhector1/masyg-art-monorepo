...
/**
 * AdminHeader — zero external UI deps (no shadcn, no lucide, no framer-motion)
 * ------------------------------------------------------------
 * Just React + Tailwind classes. Drop-in friendly.
 * Props: title, subtitle, breadcrumbs, onToggleSidebar, user, envLabel
 */

export type AdminHeaderProps = {
  title?: string;
  subtitle?: string;
  breadcrumbs?: Array<{ label: string; href?: string }>; // last is current
    ...
    <header className="sticky top-0 z-50 border-b bg-white/70 backdrop-blur supports-[backdrop-filter]:bg-white/60 dark:bg-zinc-950/60 dark:border-zinc-800">
      ...
      <div className="mx-auto flex h-16 max-w-screen-2xl items-center gap-3 px-3 sm:px-4">
              ...
              {breadcrumbs.map((bc, i) => (
                <span key={bc.label} className="flex items-center">
                  ...
                  {bc.href && i !== breadcrumbs.length - 1 ? (
                    <a href={bc.href} className="text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100">
                      {bc.label}
                    </a>
                  ) : (
                    <span className="font-medium text-zinc-900 dark:text-zinc-100">{bc.label}</span>
                  )}
                </span>
              ))}
            </nav>
        ...
        <div className="ml-auto flex items-center gap-1 sm:gap-2">
          ...
          {/* User menu shell (no lib dropdown; wire your own) */}
          <button type="button" className="rounded-xl pl-1 pr-2 hover:bg-zinc-100 dark:hover:bg-zinc-900">
            <div className="flex items-center gap-2">
      ...
      {/* Secondary row (optional): page title */}
